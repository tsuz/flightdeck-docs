...
2. **In the Tools execution function** — The corresponding execution function must exist in your consumer so there is actual code to run when the LLM calls the tool. If the LLM directs a call to a function that doesn't exist, the call will fail. See the [Building Tool Functions](/category/building-tool-functions) guide on how to implement this.
